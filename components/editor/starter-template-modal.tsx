...
function TemplatePreview({ template }: TemplatePreviewProps) {
  ...
  const maxX = Math.max(
    ...template.nodes.map((nd) => nd.position.x + (nd.width ?? 140)),
  );
  const maxY = Math.max(
    ...template.nodes.map((nd) => nd.position.y + (nd.height ?? 60)),
  );

  const bw = maxX - minX || 1;
  const bh = maxY - minY || 1;
  const scale = Math.min((VB_W - VB_PAD * 2) / bw, (VB_H - VB_PAD * 2) / bh);
    ...
    <div className="w-full" style={{ aspectRatio: `${VB_W} / ${VB_H}` }}>
      ...
      >
        ...
        {template.edges.map((edge) => {
          ...
          const sx =
            (src.position.x + (src.width ?? 140) / 2) * scale + offsetX;
          const sy =
            (src.position.y + (src.height ?? 60) / 2) * scale + offsetY;
          const tx =
            (tgt.position.x + (tgt.width ?? 140) / 2) * scale + offsetX;
          const ty =
            (tgt.position.y + (tgt.height ?? 60) / 2) * scale + offsetY;
          return (
            <line
              key={edge.id}
              x1={sx}
      ...
      <DialogContent className="max-w-[min(1520px,95vw)]! gap-0 p-0">
        <DialogHeader className="border-b border-border-default px-10 py-7">
          ...
          <DialogDescription>
            ...
            <kbd className="rounded border border-border-default bg-bg-elevated px-1 py-0.5 font-mono text-[11px] text-text-muted">
              ⌘Z
            </kbd>{" "}
            to undo.
          </DialogDescription>
